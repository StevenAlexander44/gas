...
@cache.cached(timeout=1800)
def meijer():
    meijer=prowl('https://www.meijer.com/bin/meijer/store/search/proximity?latitude=38.2&longitude=-85.7&miles=20&numToReturn=10')['store']
    result=[]
    for s in meijer:
        p=prowl(f"https://www.meijer.com/bin/meijer/store/hours?store-id={s['UnitId']}")
        if 'fuelPrices' in p:
...
def costco():
    ...
    p=prowl(f"https://www.costco.com/AjaxGetGasPricesService?warehouseid={'_'.join(warehouses)}")
    result=[]
    for s in costco:
        w=s['warehouseId']
